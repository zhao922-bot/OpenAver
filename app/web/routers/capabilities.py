...
_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "output_schema": {
            ...
            "extrafanart_written": "integer — 本次新寫成功的劇照數（非磁碟最終總數）",
            "fields_filled": "[string] — 本次補齊的欄位名",
    ...
    },
    {
        ...
        "description": "檢查配置掃描目錄內缺少有效本地劇照的影片。以磁碟 extrafanart 與 DB sample_images 本地 file URI 為準（遠端 URL 不算）。同目錄多片計入 skipped_multi。若磁碟已有劇照但 DB 陳舊，會安全同步 sample_images（不聯網、不改其他欄位）。未配置目錄時 fail-closed。",
        "method": "GET",
        "path": "/api/scraper/missing-samples",
        "input_schema": {"type": "object", "properties": {}},
        "output_schema": {
            "success": "boolean",
            "count": "integer — 缺少劇照的候選數",
            "items": "[{path, number}]",
            "skipped_multi": "integer — 同目錄多片、不進候選",
            "reconciled": "integer — 本次從磁碟同步 sample_images 的筆數",
            "error": "string|null",
        },
        "side_effect": True,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "cost_hint": "本地磁碟 + DB 掃描，不打外站；若磁碟已有劇照可能同步 sample_images",
        "_example_template": "curl -s {base}/api/scraper/missing-samples",
    },
    {
        "name": "batch_fetch_samples",

        "description": "批次補齊劇照（SSE）。只寫 extrafanart/ 與 videos.sample_images，絕不改 NFO/封面/標題/演員/檔名。已有有效本地劇照跳過且不覆蓋；同目錄多片 skipped_multi；scraper 無劇照 no_samples；下載全失敗 failed。上限 500；全域防重入 409。",
        "method": "POST",
        "path": "/api/scraper/batch-fetch-samples",
        "input_schema": {
            "type": "object",
            "properties": {
                "items": {
                    ...
                    "items": {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string", "description": "影片 path（file:/// URI，優先）"},
                            "file_path": {"type": "string", "description": "path 別名"},
                            "number": {"type": "string", "description": "忽略；以後端 DB number 為準"},
                        },
                    },
                },
                "paths": {
                    ...
                    "maxItems": 500,
                    "items": {"type": "string"},
                    "description": "可與 items 並用；去重後合計上限 500",
                },
            },
        },
        "output_schema": {
            ...
            "item": "{type:'item', current, total, number, path, status, images_written, error?}",
            "done": "{type:'done', summary:{total, success, images_downloaded, no_samples, skipped, skipped_complete, skipped_multi, failed}}",
            "status_values": "success | no_samples | failed | skipped_complete | skipped_multi",
        },
        "side_effect": True,
        "confirmation_required": True,
        "idempotent": True,
        "retry_safe": True,
        "cost_hint": "逐部打外站抓劇照；順序處理",
        "_example_template": "curl -N -X POST -H 'Content-Type: application/json' -d '{{\"paths\":[\"file:///library/IPZZ-065/IPZZ-065.mp4\"]}}' {base}/api/scraper/batch-fetch-samples",
    },
    {
        "name": "batch_enrich",
